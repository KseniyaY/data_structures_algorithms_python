...
non-constant space for creating the string which is not allowed by the problem description."""
...
Following the thoughts based on the second idea, to avoid the overflow issue of
the reverted number, what if we only revert half of the 'int' number?
After all, the reverse of the last half of the palindrome should be the same
...
from "21" to "12", and compare it with the first half of the number "12",
...
122 % 10 = 2, and if we multiply the last digit by 10 and add
...
when the original number is less than the reversed number, it means we've processed
...
class solution():
    def isPalindrome(self, x):
        ...
        while(x > revertedNumber):
            # with each step we pop one digit from x number and push 1 to a reversed one,
...
palyndromeSolution = solution()
print(palyndromeSolution.isPalindrome(323))
print(palyndromeSolution.isPalindrome(1331))
"""
Complexity Analysis
Time complexity : O(log10(n)), since we divided the input by 10 for every iteration.
